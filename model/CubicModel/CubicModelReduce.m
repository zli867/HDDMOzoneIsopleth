...
for siteNum = 1: 1
	...
	A = [];
    for i = 1: size(D, 2)
        tempA = [1, N(i), V(i), N(i)^2, N(i)*V(i), V(i)^2, N(i)^3, N(i)^2*V(i), N(i)*V(i)^2;
                0, 1, 0, 2*N(i), V(i), 0, 3*N(i)^2, 2*N(i)*V(i), V(i)^2;
                0, 0, 1, 0, N(i), 2*V(i), 0, N(i)^2, 2*N(i)*V(i);
                0, 0, 0, 0, 1, 0, 0, 2*N(i), 2*V(i);
                0, 0, 0, 2, 0, 0, 6*N(i), 2*V(i), 0;
                0, 0, 0, 0, 0, 2, 0, 0, 2*N(i)];
        A = [A; tempA];
    end
