...
for siteNum = 1: 24
    for i = 1: size(T, 1)
        ...
        if temp.Year ~= 2222 & temp.Year ~= 3333 & temp.Year ~= 4444 & strcmp(temp.SiteName, siteTableName{siteNum})
			...
			E_N = [E_N; NOx];
			E_V = [E_V; VOC];
            a0 = [a0; LCoeff(siteNum, 1)];
            a1 = [a1; LCoeff(siteNum, 2)];
            a2 = [a2; LCoeff(siteNum, 3)];
            a3 = [a3; LCoeff(siteNum, 4)];
            a4 = [a4; LCoeff(siteNum, 5)];
            a5 = [a5; LCoeff(siteNum, 6)];
        end
    end
...
fun = @(beta)log(ODV) - (a0 + a1 .* (E_N .* (beta(1) + beta(2) .* t)) + a2 .* (E_V .* (beta(3) + beta(4) .* t)) + a3 .* (E_N .* (beta(1) + beta(2) .* t)) .^2 + a4 .* (E_N .* (beta(1) + beta(2) .* t)) .* (E_V .* (beta(3) + beta(4) .* t)) + a5 .* (E_V .* (beta(3) + beta(4) .* t)) .^2);
x0 = [1, 0, 1, 0];
for i = 1:100
...
beta = result;
ODV_adjusted = exp(a0 + a1 .* (E_N .* exp(beta(1) + beta(2) .* t)) + a2 .* (E_V .* exp(beta(3) + beta(4) .* t)) + a3 .* (E_N .* exp(beta(1) + beta(2) .* t)) .^2 + a4 .* (E_N .* exp(beta(1) + beta(2) .* t)) .* (E_V .* exp(beta(3) + beta(4) .* t)) + a5 .* (E_V .* exp(beta(3) + beta(4) .* t)) .^2);
