...
for siteNum = 1: 1
    ...
    fun = @(beta)ODV - (QCoeff(siteNum, 1) + QCoeff(siteNum, 2) .* (E_N .* exp(beta(1) + beta(2) .* t)) + QCoeff(siteNum, 3) .* (E_V .* exp(beta(3) + beta(4) .* t)) + QCoeff(siteNum, 4) .* (E_N .* exp(beta(1) + beta(2) .* t)) .^2 + QCoeff(siteNum, 5) .* (E_N .* exp(beta(1) + beta(2) .* t)) .* (E_V .* exp(beta(3) + beta(4) .* t)) + QCoeff(siteNum, 6) .* (E_V .* exp(beta(3) + beta(4) .* t)) .^2);
    x0 = [1, 0, 1, 0];
    for i = 1:100
    ...
    beta = result;
    ODV_adjusted = (QCoeff(siteNum, 1) + QCoeff(siteNum, 2) .* (E_N .* exp(beta(1) + beta(2) .* t)) + QCoeff(siteNum, 3) .* (E_V .* exp(beta(3) + beta(4) .* t)) + QCoeff(siteNum, 4) .* (E_N .* exp(beta(1) + beta(2) .* t)) .^2 + QCoeff(siteNum, 5) .* (E_N .* exp(beta(1) + beta(2) .* t)) .* (E_V .* exp(beta(3) + beta(4) .* t)) + QCoeff(siteNum, 6) .* (E_V .* exp(beta(3) + beta(4) .* t)) .^2);
    figure(1)
