...
for siteNum = 1: 24
    ...
    % adjusted_N = (E_N .* (beta(1) + beta(2) .* t + beta(3) .* t.^2))
    % adjusted_V = (E_V .* (beta(4) + beta(5) .* t + beta(6) .* t.^2))
end

fun = @(beta)ODV - (a0 + a1 .* (E_N .* (beta(1) + beta(2) .* t + beta(3) .* t.^2)) + a2 .* (E_V .* (beta(4) + beta(5) .* t + beta(6) .* t.^2)) + ...
      a3 .* (E_N .* (beta(1) + beta(2) .* t + beta(3) .* t.^2)) .^2 +...
      a4 .* (E_N .* (beta(1) + beta(2) .* t + beta(3) .* t.^2)) .* (E_V .* (beta(4) + beta(5) .* t + beta(6) .* t.^2)) + ...
      a5 .* (E_V .* (beta(4) + beta(5) .* t + beta(6) .* t.^2)) .^2);
x0 = [1, 0, 0, 1, 0, 0];
for i = 1:100
...
beta = result;
ODV_adjusted = (a0 + a1 .* (E_N .* (beta(1) + beta(2) .* t + beta(3) .* t.^2)) + a2 .* (E_V .* (beta(4) + beta(5) .* t + beta(6) .* t.^2)) + ...
    a3 .* (E_N .* (beta(1) + beta(2) .* t + beta(3) .* t.^2)) .^2 + ...
    a4 .* (E_N .* (beta(1) + beta(2) .* t + beta(3) .* t.^2)) .* (E_V .* (beta(4) + beta(5) .* t + beta(6) .* t.^2)) +...
    a5 .* (E_V .* (beta(4) + beta(5) .* t + beta(6) .* t.^2)) .^2);
...
figure(4)
time = [min(t):1:max(t)];
ratio_N = (beta(1) + beta(2) .* time + beta(3) .* time.^2);
ratio_V = (beta(4) + beta(5) .* time + beta(6) .* time.^2);
