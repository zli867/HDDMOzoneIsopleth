...
for i = 1: length(allYear)
    ...
    a4_tmp = a4(index);
    a5_tmp = a5(index);
    fun = @(beta)((ODV_tmp - (a0_tmp + a1_tmp .* beta(1) + a2_tmp .* beta(2) + a3_tmp .* beta(1) .^2 + a4_tmp .* beta(1) .* beta(2) + a5_tmp .* beta(2) .^2))./ODV_tmp).^2 + ...
            ((beta(1) - NOx_tmp)./NOx_tmp).^2 + ((beta(2) -VOC_tmp)./VOC_tmp).^2;
...
figure;
plot(N_adj, 'r*--'), hold on
plot(E_N_ori, 'b+--')
figure;
plot(V_adj, 'r*--'), hold on
plot(E_V_ori, 'b+--')