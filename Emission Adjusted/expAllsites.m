...
fun = @(beta)ODV - (a0 + a1 .* (E_N .* exp(beta(1) + beta(2) .* t)) + a2 .* (E_V .* exp(beta(3) + beta(4) .* t)) + a3 .* (E_N .* exp(beta(1) + beta(2) .* t)) .^2 + a4 .* (E_N .* exp(beta(1) + beta(2) .* t)) .* (E_V .* exp(beta(3) + beta(4) .* t)) + a5 .* exp(E_V .* (beta(3) + beta(4) .* t)) .^2);
x0 = [0, 0, 0, 0];
for i = 1:100
