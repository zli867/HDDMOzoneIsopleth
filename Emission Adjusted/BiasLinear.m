...
fun = @(beta)((ODV - (a0 + a1 .* (E_N .* (beta(1) + beta(2) .* t)) + a2 .* (E_V .* (beta(3) + beta(4) .* t)) + ...
                a3 .* (E_N .* (beta(1) + beta(2) .* t)).^2 + a4 .* (E_N .* (beta(1) + beta(2) .* t)) .* (E_V .* (beta(3) + beta(4) .* t)) + a5 .* (E_V .* (beta(3) + beta(4) .* t)).^2))./ODV).^2 + ...
                ((beta(1) + beta(2) .* t - 1)./(beta(1) + beta(2) .* t)).^2 + ((beta(3) + beta(4) .* t - 1)./(beta(3) + beta(4) .* t)) .^2;
x0 = [1, 0, 1, 0];
for i = 1:100
...
adj_N = (E_N_ori .* (beta(1) + beta(2) .* allYear));
adj_V = (E_V_ori .* (beta(3) + beta(4) .* allYear));
figure;
