...
for siteNum = 1: 24
    ...
    % weight 6 (ozone + derivatives) * 15 (data points)
    W_diag = ones(90, 1);
    % for base and near points
    baseAndNear = [2, 9, 10, 11, 12];
    for point = baseAndNear
        for i = 6* (point - 1) + 1: 6* (point - 1) + 6
            W_diag(i) = 1/5;
        end
    end
    ...
    for point = boader
        for i = 6* (point - 1) + 1: 6* (point - 1) + 6
            modeIndex = mod(i, 6);
            if modeIndex == 2 || modeIndex == 4 || modeIndex == 5
                W_diag(i) = 1/3;
            end
        end
    end
    ...
    % (A' * W * A) * beta_est = A' * W * b;
	beta = inv(A' * W * A) * (A' * W * b);
    ...
    ylim([0, 1800])
	% save figures
	% saveas(gcf, strcat('/Users/zongrunli/Desktop/DDM-CMAQ Updates/figures top 4/', siteName{siteNum}, ' Qmodel.jpg'))
end