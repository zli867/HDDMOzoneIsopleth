...
for siteNum = 11
    ...
    % Input the weight w1 -> O3 ; w2 -> dO3/dN; w3 -> dO3/dV; w4 -> dO3/dNdV; w5 -> dO3/dN^2; w6 -> dO3/dV^2
    % w = [1, 1, 1, 1, 1, 1]; % original quadratic model
    % w = [1, 0, 0, 0, 0, 0]; % only use ozone do least square
    % w = [1, 1, 1, 0, 0, 0]; % only use ozone and first order deravitive
    w = [1/std(D(1,:))^2, 1/std(D(2,:))^2, 1/std(D(3, :))^2, 1/std(D(4, :))^2, 1/std(D(5,:))^2, 1/std(D(6,:))^2];
	...
	O3 = a0 + a1 * NOx + a2 * VOC + a3 * NOx.^2 + a4 * NOx .* VOC + a5 * VOC.^2;
	QCoeff = [QCoeff; a0, a1, a2, a3, a4, a5];
	...
	hold off;
	% save figures
	% saveas(gcf, strcat('/Users/zongrunli/Desktop/CMAQ-DDM/figures/', siteName{siteNum}, ' QWmodel.jpg'))
end